...
def test_signals(sim_det):
    ...
    assert (
        sim_det.drv.acquire_time.source == "mock+ca://255idSimDet:cam1:AcquireTime_RBV"
    )
    assert (
        sim_det.drv.detector_state.source
        == "mock+ca://255idSimDet:cam1:DetectorState_RBV"
    )
    ...
    assert (
        sim_det.overlays[0].center_x.source
        == "mock+ca://255idSimDet:Over1:1:CenterX_RBV"
    )
    assert (
        sim_det.overlays[0].center_y.source
        == "mock+ca://255idSimDet:Over1:1:CenterY_RBV"
    )
    assert (
        sim_det.overlays[0].size_x.source == "mock+ca://255idSimDet:Over1:1:SizeX_RBV"
    )
    assert (
        sim_det.overlays[0].size_y.source == "mock+ca://255idSimDet:Over1:1:SizeY_RBV"
    )
